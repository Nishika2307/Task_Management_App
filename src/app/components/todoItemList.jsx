...
function TodoItemList(props) {
  ...
  const removeTodo = (event, _list, index) => {
    event.stopPropagation();
  ...
  if (props.list.id) {
    return (
      <Container maxWidth="lg" className={classes.container}>
        ...
        {props.list.todoList.length === 0 &&
        <div>
          <h5 style={{textAlign: 'center', padding: 25}}>If there is no Todo Item in your list, you can add a new Todo Item with the button below.</h5>
        </div>
        }
        <IconButton aria-label="edit" className={classes.addTodoItemButton} onClick={(_event) => {
    ...
    return (
      <div>
        <h3 style={{textAlign: 'center', padding: 25}}>No saved Todo List was found, please click on the first Todo list from the left menu.
          Add your List.</h3>
      </div>
    )
  }
